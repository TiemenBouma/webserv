...
/*                                                   +#+                      */
/*   Created: 2023/02/08 10:52:09 by svos          #+#    #+#                 */
/*   Updated: 2023/02/08 15:17:26 by svos          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	ConfigServer::_next_directive(std::string::iterator &it)
{
	...
	while (*it != '}' && *it != '\n' && *it != '\0')
	{
		// std::cout << "*it int while loop: '" << (int)*it << "'" << std::endl;
		// std::cout << "*it int while loop: '" << *it << "'" << std::endl;
		it++;
	}
	while (isspace(*it) != 0)
		it++;
	...
	while (*it != '}' && *it != '\0')
	{
		...
		switch (i)
		{
			...
			case REDIRECTION:
				...
				// std::cout << "in data class: " << locations[0].location << std::endl;
				break;
			default:
				throw(UnknownKeyword());
		}
...
int	ConfigServer::cmp_directive(std::string::iterator it, std::string directive)
{
	...
	// std::cout << "checking if directive is a " << directive << std::endl;
